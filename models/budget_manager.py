...
class BudgetManager:
    ...
    def get_all_transactions(self, db_session, user_id):
        """獲取指定使用者的所有交易紀錄"""
    ...
    def add_transaction(self, db_session, user_id, date, item, amount, transaction_type, budget_category, description=""):
        """新增一筆交易紀錄，並綁定使用者"""
        if amount <= 0:
    ...
    def delete_transaction(self, db_session, user_id, transaction_id):
        """刪除一筆交易，並驗證使用者"""
    ...
    def get_all_transaction_months(self, db_session, user_id):
    ...
    def set_budget(self, db_session, user_id, month, category, amount, notes=""):
    ...
    def delete_budget(self, db_session, user_id, month, category):
        """刪除某位使用者在某月某類別的預算"""
    ...
    def get_all_budget_categories(self, db_session, user_id):
        """獲取指定使用者所有已設定預算的類別"""
    ...
    def calculate_monthly_expenses(self, db_session, user_id, year_month):
        """使用 SQL彙總指定使用者在某月的支出"""
        stmt = (
    ...
    def get_transactions_by_category_over_time(self, db_session, user_id, interval='month'):
    ...
    def check_over_warnings(self, db_session, user_id, month=None):
        """檢查指定使用者的超支警告"""
        ...
        expenses_result = db_session.execute(expense_stmt)
