...
- 目前可進入使用觀察與 Phase 6 規劃；若後續測試發現阻塞級 bug，仍先以低風險修補處理。
...
- 目前提醒包含：信用卡負數待核對、預算超支、預算即將用完、旅行帳本核對入口與本月最大支出。
...
| Phase 6 多人旅行與商品化基礎 | 第一輪開始 | Nomica Insights 已納入首頁；Money Flow 移到 Phase 7，每位成員統計偏好、帳號 provider、iOS/PWA 登入、timezone、匯率與投資/資金分配獨立頁列入後續評估 |
